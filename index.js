...
if (isLiking && discussion.author && discussion.author._id) {
  ...
  if (user) {
    await createNotification({
      ...
      message: `أعجب ${user.name} بنقاشك "${discussion.title}"`,
...
if (isReply) {
  ...
  if (parentComment && parentComment.author && parentComment.author._id) {
    await createNotification({
      ...
      message: `ردّ ${user.name} على تعليقك في "${discussion.title}"`,
      link: `/discussion/${req.params.id}`,
      metadata: {
        discussionId: discussion._id,
  ...
  if (discussion.author && discussion.author._id) {
    await createNotification({
      ...
      message: `علّق ${user.name} على "${discussion.title}"`,
      link: `/discussion/${req.params.id}`,
      metadata: {
        discussionId: discussion._id,
        commentId: newComment._id,
  ...
  if (isUpvoting) {
    ...
    if (user && discussion) {
      await createNotification({
        ...
        message: `أعجب ${user.name} بتعليقك في "${discussion.title}"`,
...
if (isAdding && comment.author._id.toString() !== userId) {
  ...
  if (positiveReactions.includes(reactionType)) {
    const user = await User.findById(userId);
    if (user && discussion) {
      const reactionLabels = { logical: 'منطقياً', inspiring: 'ملهماً' };
      await createNotification({
        recipient: comment.author._id,
        sender: user,
        type: reactionType === 'logical' ? 'reaction_logical' : 'reaction_inspiring',
        title: 'تفاعل إيجابي مع تعليقك',
        message: `وجد ${user.name} تعليقك ${reactionLabels[reactionType]} في "${discussion.title}"`,
        link: `/discussion/${comment.discussionId}`,
        metadata: {
          discussionId: comment.discussionId,
          commentId: comment._id,
...
if (circle.isPrivate) {
  ...
  if (circle.createdBy && circle.createdBy._id) {
    await createNotification({
      ...
      message: `طلب ${user ? user.name : 'مستخدم'} الانضمام لدائرة "${circle.name}"`,
...
await createNotification({
  ...
  message: `وافق ${user.name} على انضمامك لدائرة "${circle.name}"`,
  link: `/circles/${circle._id}`,
  metadata: { circleId: circle._id },
});
...
await createNotification({
  ...
  title: 'تحديث على طلب الانضمام',
  message: `طلبك للانضمام إلى "${circle.name}" لم يتم قبوله حالياً`,
  link: `/circles`,
  metadata: { circleId: circle._id },
});
